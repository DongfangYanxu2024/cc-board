...
> 当前版本为 `0.1.1` 预览版。建议先在测试项目中使用并保留重要文件的版本控制或备份。
...
1. 从 GitHub Releases 下载 `cc-board-Setup-0.1.1.exe` 并安装。
...
“完全自动”对应 Claude Code 的 `bypassPermissions` 模式。每次启动前都会显示风险确认。开启后，智能体可以无需逐项确认地执行命令、修改或删除文件以及联网，可能造成数据丢失、信息泄露和额外费用。工作文件夹不是安全沙箱；关闭模式或停止任务不会撤销已经完成的操作。
