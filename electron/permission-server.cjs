...
readline.createInterface({ input: process.stdin }).on('line', async line => {
  ...
  if (m.method === 'initialize') return respond(m.id, { protocolVersion: m.params?.protocolVersion || '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'cc-board-permissions', version: process.env.CCB_VERSION || '0.3.1' } });
